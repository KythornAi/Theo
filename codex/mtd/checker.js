...
function getIncomeRange(bandId, exact) {
  ...
  const band = INCOME_BANDS.find(b => b.id === bandId);
  if (!band || band.flag === 'unsure' || band.min === null) return null;
  return { min: band.min, max: band.max }; // over50 has max: Infinity, which is fine
}

// Combined qualifying-income range across self-employment + property.
function computeQualifyingRange() {
  const seSel = state.roles.includes('sole-trader');
  const propSel = state.roles.includes('uk-property') || state.roles.includes('foreign-property');
  const se = seSel ? getIncomeRange(state.seIncomeBand, state.seIncomeExact) : { min: 0, max: 0 };
  const prop = propSel ? getIncomeRange(state.propIncomeBand, state.propIncomeExact) : { min: 0, max: 0 };
  if (se === null || prop === null) return null; // an answer is "not sure"
  return { min: se.min + prop.min, max: se.max + prop.max };
}
...
function determineResult() {
  ...
  const range = computeQualifyingRange();

  if (thresholdRow && range !== null) {
    const t = thresholdRow.threshold;
    // Exactly on the threshold from an exact figure: HMRC's test is "more than",
    // so it is not in scope, but nudge them to double-check.
    if (range.min === t && range.max === t) {
    ...
    if (range.min > t) {
      ...
      // Range straddles the threshold -> cannot say confidently.
      return buildResult('G', qi, edgeFlags, { straddle: true, thresholdRow });
    }
  }
...
function buildS0(el) {
  ...
  stepTitle.className = 'screen-step-title';
  stepTitle.textContent = 'About your check';
  el.appendChild(stepTitle);
  q(el, 'What are you checking?');
  helper(el, 'This checker is for Making Tax Digital for <strong>Income Tax</strong>, not VAT.');
  const w = optsWrap(el);
  w.classList.add('opts-s0');
  option(w, 'self', 'MTD for Income Tax, for myself');
  option(w, 'client', 'MTD for Income Tax, for someone else');
  option(w, 'vat', 'MTD for VAT');
  option(w, 'unsure', "I'm not sure");
...
function buildS1(el) {
  const stepTitle = document.createElement('div');
  stepTitle.className = 'screen-step-title';
  stepTitle.textContent = 'Your income';
  el.appendChild(stepTitle);
  q(el, 'Which of these describe you?');
  helper(el, 'Select all that apply. We use this to work out your qualifying income.');
  ...
  fn.className = 'opts-footnote';
  fn.innerHTML = 'PAYE, dividends and pensions don\'t count toward the MTD threshold, but may still matter for your tax return.';
  el.appendChild(fn);
}
...
function buildS3(el) {
  const stepTitle = document.createElement('div');
  stepTitle.className = 'screen-step-title';
  stepTitle.textContent = 'Your figures';
  el.appendChild(stepTitle);
  q(el, 'Which figures are you using?');
  helper(el, 'HMRC uses the figure from a specific tax return to decide your start date.');
  const w = optsWrap(el);
  w.classList.add('opts-s0');
  option(w, '2024/25', '2024 to 2025 tax return');
  option(w, '2025/26', '2025 to 2026 tax return');
  option(w, '2026/27', '2026 to 2027 tax return');
  option(w, 'estimate', 'A current estimate (not from a filed return)');
  option(w, 'unsure', 'Not sure');
  w.querySelectorAll('.opt').forEach(b => b.addEventListener('click', () => {
    w.querySelectorAll('.opt').forEach(o => { o.setAttribute('aria-checked','false'); o.classList.remove('selected'); });
    b.setAttribute('aria-checked','true'); b.classList.add('selected');
...
function buildS3b(el) {
  ...
  option(w, 'shorter', 'Shorter than 12 months');
  option(w, 'longer', 'Longer than 12 months');
  option(w, 'unsure', 'Not sure');
  w.querySelectorAll('.opt').forEach(b => b.addEventListener('click', () => {
    w.querySelectorAll('.opt').forEach(o => { o.setAttribute('aria-checked','false'); o.classList.remove('selected'); });
    b.setAttribute('aria-checked','true'); b.classList.add('selected');
...
function buildS7(el) {
  ...
  const w = optsWrap(el, true);
  option(w, 'digital', 'I may be digitally excluded', null, true);
...
function renderResult() {
  ...
  // Straddle warning for banded income that could go either side of a threshold.
  if (result.extra.straddle && result.extra.thresholdRow) {
    const panel = document.createElement('div');
    panel.className = 'callout warning';
    panel.innerHTML = '<p>Based on the income bands you chose, your qualifying income could be either side of the £' + result.extra.thresholdRow.threshold.toLocaleString('en-GB') + ' threshold. Enter an exact figure, or check the <a href="' + HMRC_SOURCES.qualifying + '" target="_blank" rel="noopener noreferrer">HMRC qualifying-income guidance</a>, to be sure.</p>';
    resEl.appendChild(panel);
  }
